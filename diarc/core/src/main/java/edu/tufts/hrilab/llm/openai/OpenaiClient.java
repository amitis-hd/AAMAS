...
public class OpenaiClient {
  ...
  private String responsesEndpoint = "https://api.openai.com/v1/responses";
  ...
  public OpenaiResponses createResponse(String model, String input, String responseId) {
    if (apiKey == null) {
      log.error("Cannot send request without an API key for OpenAI.");
      return null;
    }

    OpenaiResponsesRequestBody requestBody = new OpenaiResponsesRequestBody(model, input, responseId);
    ...
    Gson gson = new Gson();
    log.debug("Sending /responses request: " + gson.toJson(requestBody));

    String responseJson = Http.sendPostRequest(responsesEndpoint, requestBody, headers);

    log.info("Response from OpenAI /responses: " + responseJson);
    return gson.fromJson(responseJson, OpenaiResponses.class);
  }
}